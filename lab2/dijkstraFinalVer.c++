...
void dijkstra(Graph g, int source){
    ...
    while(!isEmpty(PQ)){
        ...
        s[u-1] = 1;
        for (v=0; v<len; v++){
            // printf("in for loop, v=%d\n", v+1);
            if (g.AdjM[u-1][v]>0){
                // printf("d[v] is: %d, d[u] is: %d, edge is: %d\n", d[v], d[u-1], g.AdjM[u-1][v]);
...
void printPQ(int* PQ){ //The nodes are sorted from highest priority to lowest priority
...
//remove vertice that is passed in
void dequeue(int v, int*PQ){
    if (PQ[PQsize-1]==v){ //there is only one element
        PQ[PQsize-1]=0;
        PQsize--;
    }
    else{
